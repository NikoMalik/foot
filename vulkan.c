...
void vk_unref(struct vk_buffer *_buf) {
    if (_buf == NULL)
        return;
...
buffer_release(void *data, struct wl_buffer *wl_buffer) {
    ...
    LOG_DBG("buffer_release: %p, ref_count=%zu", (void *)buffer, buffer->ref_count);
...
vk_buffer_create(struct vk_buffer_chain *chain, int width, int height, bool with_alpha, bool immediate_purge) {
    ...
    *img = (struct vk_buffer_private){
    ...
    if (img->public.wl_buf == NULL) {
        LOG_ERR("Could not create wl_buffer");
        goto error;
    }
...
vk_get_buffer(struct vk_buffer_chain *chain, int width, int height, bool with_alpha) {
    ...
    if (cached != NULL) {
        LOG_DBG("re-using buffer %p from cache (age=%u)", (void *)cached, cached->public.age);
        cached->busy = true;
    ...
    LOG_DBG("creating new buffer %dx%d", width, height);
    return vk_buffer_create(chain, width, height, with_alpha, false);
}

void vk_did_not_use_buf(struct vk_buffer *_buf) {
    struct vk_buffer_private *buf = (struct vk_buffer_private *)_buf;
    buf->busy = false;
}
...
void vk_get_many(
    ...
    for (size_t idx = 0; idx < count; idx++)
        bufs[idx] = vk_buffer_create(chain, widths[idx], heights[idx], with_alpha, true);
}
